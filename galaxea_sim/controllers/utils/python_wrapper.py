...
lib = ctypes.cdll.LoadLibrary(dir_path + '/librelaxed_ik_lib.so')
...
class RelaxedIKRust:
    def __init__(self, setting_file_path = None):
        ...
        if setting_file_path is None:
            ...
            # set chdir to the parent directory of the comfig dir
            original_dir = os.getcwd()
            print("original dir:", original_dir)
            os.chdir(os.path.dirname( os.path.dirname(setting_file_path)))
            self.obj = lib.relaxed_ik_new(ctypes.c_char_p(setting_file_path.encode('utf-8')))
