...
class BlockHammerBeatEnv(RoboTwinBaseEnv):
    ...
    def reset_world(self, reset_info=None):
        ...
        if reset_info is not None:
            self.hammer.set_pose(
                sapien.Pose(
                    reset_info["hammer_pose"][:3], reset_info["hammer_pose"][3:]
                )
            )
            self.block.set_pose(
                sapien.Pose(reset_info["block_pose"][:3], reset_info["block_pose"][3:])
            )

    def solution(self):
        action_mode = "left" if self.block_pose_wrt_table.p[1] > 0 else "right"
        pose1 = get_grasp_pose_w_labeled_direction(
            self.hammer, self.hammer_data, pre_dis=0.1
        )  # pre grasp pose
        print(f"Pre-grasp pose: {pose1}")
        yield ("open_gripper", {"action_mode": action_mode})
        yield ("move_to_pose", {f"{action_mode}_pose": deepcopy(pose1)})
        pose2 = get_grasp_pose_w_labeled_direction(
            self.hammer, self.hammer_data, pre_dis=-0.075
        )  # grap pose
        ...
        pose4 = get_target_pose_from_goal_point_and_direction(
            ...
            self.hammer_data,
            (
                self.robot.left_ee_link
                if action_mode == "left"
                else self.robot.right_ee_link
            ),
    ...
    def _get_info(self):
        ...
        success = (
            np.all(np.abs(hammer_target_pose[:2] - block_pose[:2]) < eps)
            and hammer_target_pose[2] < self.table_height + 0.07
            and hammer_target_pose[2] > self.table_height + 0.04
        )
        return dict(
    ...
    def _get_reset_info(self):
        return dict(
            hammer_pose=np.concatenate(
                [self.hammer.get_pose().p, self.hammer.get_pose().q], axis=0
            ),
            block_pose=np.concatenate(
                [self.block.get_pose().p, self.block.get_pose().q], axis=0
            ),
        )
