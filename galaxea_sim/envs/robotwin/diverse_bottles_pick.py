...
from .dual_bottles_pick_easy_traj_aug import DualBottlesPickEasyEnv
...
class DiverseBottlesPickEnv(DualBottlesPickEasyEnv):
    ...
    def _setup_bottle1(self, bottle_id=None):
        ...
        self.bottle1, self.bottle1_data = rand_create_glb(
            ...
            rotate_rand=False,
            qpos=(
                sapien.Pose(q=[0.92387, 0, 0, 0.38268])
                * sapien.Pose(q=[0.707, 0.707, 0, 0])
            ).q,
    ...
    def get_object_dict(self):
        return dict(
            ...
            bottle1_id=np.array(
                [id == self._bottle1_id for id in self.id_list], dtype=np.float32
            ),
            bottle2_id=np.array(
                [id == self._bottle2_id for id in self.id_list], dtype=np.float32
            ),
        )
