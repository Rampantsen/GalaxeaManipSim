...
class DualBottlesPickEasyEnv(RoboTwinBaseEnv):
    ...
    def _setup_green_bottle(self):
        self.green_bottle_xlim = [-0.2, 0.0]
    ...
    def reset_world(self, reset_info=None):
        ...
        if reset_info is not None:
            self.red_bottle.set_pose(
                sapien.Pose(
                    reset_info["red_bottle_initial_pos_on_table"], self.red_bottle_qpos
                )
            )
            self.green_bottle.set_pose(
                sapien.Pose(
                    reset_info["green_bottle_initial_pos_on_table"],
                    self.green_bottle_qpos,
                )
            )
            self.green_bottle_initial_pose_on_table = self.green_bottle.get_pose()
            self.red_bottle_initial_pose_on_table = self.red_bottle.get_pose()
    ...
    def left_target_pose(self):
        ...
        (target_pose := Pose(p=p)).set_rpy(
            rpy=np.array([np.pi, np.pi, np.pi / 2], dtype=np.float32)
            + self.robot.left_ee_rpy_offset
        )
        return target_pose
    ...
    def right_target_pose(self):
        if self.robot_name == "r1_pro":
            ...
            p = [0.6, -0.13, 1.2]
        (target_pose := Pose(p=p)).set_rpy(
            rpy=np.array([0, 0, np.pi / 2], dtype=np.float32)
    ...
    def solution(self):
        (left_grasp_ori := Pose()).set_rpy(
            rpy=(
                np.array([np.pi, -np.pi, 2.26], dtype=np.float32)
                + self.robot.left_ee_rpy_offset
            )
        )
        (right_grasp_ori := Pose()).set_rpy(
            rpy=(
                np.array([0, 0, 0.88], dtype=np.float32)
                + self.robot.right_ee_rpy_offset
            )
        )
        left_pose0 = Pose(
            p=self.green_bottle.get_pose().p + [-0.1096, 0.1164, 0.0],
            q=left_grasp_ori.q,
        )
        right_pose0 = Pose(
            p=self.red_bottle.get_pose().p + [-0.1096, -0.1164, 0.0],
            q=right_grasp_ori.q,
        )
        left_pose1 = Pose(
            p=self.green_bottle.get_pose().p + [-0.0196, 0.0164, 0.0],
            q=left_grasp_ori.q,
        )
        right_pose1 = Pose(
            p=self.red_bottle.get_pose().p + [-0.0196, -0.0164, 0.0],
            q=right_grasp_ori.q,
        )
        substeps = [
            (
    ...
    def _get_info(self):
        left_distance = np.linalg.norm(
            self.green_bottle.get_pose().p - self.left_target_pose.p
        )
        right_distance = np.linalg.norm(
            self.red_bottle.get_pose().p - self.right_target_pose.p
        )
        left_height = self.green_bottle.get_pose().p[2].item()
        right_height = self.red_bottle.get_pose().p[2].item()
        left_success = (
            left_distance < 0.15
            and left_height >= self.left_target_pose.p[2].item() - 0.07
        )
        right_success = (
            right_distance < 0.15
            and right_height >= self.right_target_pose.p[2].item() - 0.07
        )
    ...
    def _get_reset_info(self):
        return dict(
            red_bottle_initial_pos_on_table=copy(
                self.red_bottle_initial_pose_on_table.p
            ),
            green_bottle_initial_pos_on_table=copy(
                self.green_bottle_initial_pose_on_table.p
            ),
        )
    ...
    def get_object_dict(self):
        return dict(
            red_bottle=np.concatenate(
                [self.red_bottle.get_pose().p, self.red_bottle.get_pose().q]
            ),
            green_bottle=np.concatenate(
                [self.green_bottle.get_pose().p, self.green_bottle.get_pose().q]
            ),
        )
