...
class RoboTwinBaseEnv(BimanualManipulationEnv):
    def __init__(
        ...
        self.table_type = table_type  # 保存 table_type
        super().__init__(
            robot_class,
            robot_kwargs,
            controller_type,
    ...
    def _build_world(self):
        super()._build_world()
        self._setup_table(self.table_type)  # 传递 table_type 参数
        self._setup_wall()
        self.reset_world()
    ...
    def _setup_table(self,table_type):
        ...
        if table_type is None or table_type == "plain":
            ...
            # 未知的 table_type，输出警告并使用默认纯色桌子
            print(f"Warning: Unknown table_type '{table_type}', using plain table.")
            self.table = create_table(
                self._scene,
                sapien.Pose(p=self.tabletop_center_in_world),
                length=self.table_length,
