...
def compute_grasp_poses(
    ...
    pcd = mesh.sample_points_uniformly(number_of_points=n_pts, seed=seed)
    ...
    for grasp_pose, closing_dist in zip(grasp_poses, closing_dists):
        ...
        for w in open_widths:
            ...
            if gripper.isCollisionFree(grasp_qpos):
                ...
                # sapien format
                pq = pq_from_transform(grasp_pose)
    ...
    if len(valid_grasp_poses) > 0 and vis:
        ...
        gripper.play(valid_grasp_qpos.T, dt=0.5)
...
def main():
    ...
    from mani_skill2.envs.pick_and_place.pick_single import YCB_DIR
